...
def rk4(xnaught = None,
    ...
    for t in times:
        ...
        #when the ball hits the ground, stop computing
        if currentPos[1] <= 0:
            break
...
if __name__ == "__main__":
    ...
    #compute distance via RK4
    distance = rk4(xnaught=x0,
                   vnaught=v0,
                   times = times,
                   h = timeStep,
    ...
    print('The baseball lands', round(distance[0],2), 'meters',
          'away when thrown at', speedmph, 'mph (%.2f m/s)'%(speedmph*mphToms))
